...
const randomPassword = () => {
  updateTime = Date.now();
  currentPassword = getRandomPassword(20);
...
const checkPort = async () => {
  ...
  if(!exists) {
    ...
    if(changePasswordMark) {
      ...
      qrcode = 'ss://' + Buffer.from(`${ method }:${ currentPassword }`).toString('base64') + '@${ address }:${ currentPort }' + '/?plugin=obfs-local%3bobfs%3dtls%3bobfs-host%3dwww.apple.com';
    }
  }
};
