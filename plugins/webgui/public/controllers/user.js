...
.controller('UserAccountController', ['$scope', '$http', '$mdMedia', 'userApi', '$filter', 'payDialog', 'qrcodeDialog', '$interval', '$localStorage', 'changePasswordDialog', 'payByGiftCardDialog', 'subscribeDialog', '$q', '$state', 'wireGuardConfigDialog',
  ($scope, $http, $mdMedia, userApi, $filter, payDialog, qrcodeDialog, $interval, $localStorage, changePasswordDialog, payByGiftCardDialog, subscribeDialog, $q, $state, wireGuardConfigDialog) => {
    ...
    if(String(navigator.platform).toLowerCase().indexOf("linux") > -1) {
	    ...
	    $scope.os = 'ios';
    } else if(/(Win32|Windows|Win16)/i.test(navigator.platform)){
	    $scope.os = 'win';
    } else if(/(Mac68K|MacPPC|MacIntel)/i.test(navigator.platform)){
	...
	if(server.comment == 'http' || server.comment == 'tls') { var str = base64Encode(server.method + ':' + account.password) + '@' + server.host + ':' + (account.port + server.shift); str = str.replace(/\//g , '_'); return 'ss://' + str + '/?plugin=obfs-local%3bobfs%3d' + server.comment + '%3bobfs-host%3dwww.apple.com'; }
        else { var str = base64Encode(server.method + ':' + account.password) + '@' + server.host + ':' + (account.port + server.shift); str = str.replace(/\//g , '_'); return 'ss://' + str; }
      }
    };
    ...
    $scope.clipboardSuccess = event => {
      $scope.toast($filter('translate')('复制成功'));
    };
    $scope.isWG = server => {
